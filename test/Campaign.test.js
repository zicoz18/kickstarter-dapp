...
beforeEach(async () => {
    ...
    // when we send a transaction to deploya a contract we get back the contract
    // but when we send a transaction to edit data on the blockchain we do not get back any data other than txHash
...
describe('Campaigns', async () => {
    ...
    it('allows people to contribute and marks them as approvers', async () => {
        const isNotAContributor = await campaign.methods.approvers(accounts[1]).call();
        assert(!isNotAContributor);
